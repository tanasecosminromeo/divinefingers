...
export default (function INIT() {
  class Client extends EventEmitter {
    constructor() {
      ...
      self.watch = {
        ...
        addListeners() {
          ...
          atom.notifications.addInfo('Added watch listeners', {
            dismissable: false,
          });
        ...
        removeListeners() {
          if (self.watcher != null) {
            self.watcher.close();
            atom.notifications.addInfo('Stopped watch listeners', {
              dismissable: false,
            });
          }
        },
    ...
    readConfig(callback) {
      ...
      if (self.ftpConfigPath === false) throw new Error('getConfigPath returned false, but expected a string');
    ...
    onceConnected(onconnect) {
      ...
      console.warn(`divinefingers: Not connected and typeof onconnect is ${typeof onconnect}`);
      return false;
    }
    ...
    doConnect() {
      ...
      atom.notifications.addInfo('Connecting to<br /><small>'+self.info.user+'@'+self.info.host+':'+(self.info.port || 21)+'<br /><small>'+self.info.remote+self.info.bundle+'</small></small>', {
        dismissable: false,
      });
			global.dff.divineStatusBar.setStatusTo(self.info.remote.split("/").pop(), "icon icon-sync", "text-info");
      ...
      switch (self.info.protocol) {
        ...
        default:
          throw new Error('No `protocol` found in connection credential. Please recreate .ftpconfig file from Packages -> divinefingers -> Create (S)FTP config file.');
      }
      ...
      self.connector.connect(info, () => {
        ...
        self.status = 'CONNECTED';
				...
				global.dff.divineStatusBar.setStatusTo(self.info.remote.split("/").pop(), "icon icon-globe", "text-success");

        atom.notifications.addSuccess('Connected', {
          dismissable: false,
        });
      });
      ...
      self.connector.on('closed', (action) => {
        ...
        self.emit('closed');
				...
				global.dff.divineStatusBar.setStatusTo();
      ...
      self.connector.on('error', (err) => {
        atom.notifications.addError(`Oups, ${err}`, {
          dismissable: false,
        });
    ...
    toLocal(remote) {
      ...
      if (typeof remote !== 'string') {
        throw new Error(`: remote must be a string, was passed ${typeof remote}`);
      }
    ...
    _next() {
      ...
      atom.project.divinefingers.emit('queue-changed');
    }
