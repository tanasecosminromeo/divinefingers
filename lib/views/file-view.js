...
module.exports = FileView = (function (parent) {
	...
	FileView.prototype.initialize = function (file) {
		...
		self.on('mousedown', function (e) {
			...
			var $selected = $('.divinefingers-view .selected');
			...
			if ( (button === 0 || button === 2) && !(button === 2 && $selected.length > 1) ) {
				if ( !e[selectKey]) {
					...
					$('.divinefingers-view .entries.list-tree').removeClass('multi-select');
				} else {
					$('.divinefingers-view .entries.list-tree').addClass('multi-select');
				}
