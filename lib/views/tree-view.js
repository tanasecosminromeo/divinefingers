	...
	if(treeView && typeof treeView.detach === "function"){ // Fix for Issue 433 ( workaround to stop throwing error)
		try{
			...
			if(hideLocalTreeError === false){
				atom.notifications.addWarning("See issue #433", {
					dismissable: false,
				});
...
module.exports = TreeView = (function (parent) {
	...
	TreeView.content = function () {
		return this.div({
			'class': 'divinefingers-view ftptree-view-resizer tool-panel',
	...
	TreeView.prototype.initialize = function (state) {
		...
		if (atom.project.divinefingers.isConnected())
			self.showOnline();
		else
			self.showOffline();

		self.root = new DirectoryView(atom.project.divinefingers.root);
		self.root.expand();
		...
		atom.config.onDidChange('divinefingers.hideLocalWhenDisplayed', function (values) {
			if (values.newValue) {
				if (self.isVisible()) {
					hideLocalTree();
				}
		...
		atom.project.divinefingers.on('debug', function (msg) {
			self.debug.prepend('<li>'+msg+'</li>');
			var children = self.debug.children();
			if (children.length > 20)
				children.last().remove();
		});
		atom.project.divinefingers.on('queue-changed', function () {
			self.progress.empty();

			var queue = [];
			if (atom.project.divinefingers._current)
				queue.push(atom.project.divinefingers._current);
			for (var i = 0, l = atom.project.divinefingers._queue.length; i < l; ++i)
				queue.push(atom.project.divinefingers._queue[i]);
		...
		self.offline.on('click', '[role="configure"]', function (e) {
			atom.workspace.open(atom.project.divinefingers.getConfigPath());
		});
		...
		atom.project.divinefingers.on('connected', function () {
			self.showOnline();
		});
		//atom.project.divinefingers.on('closed', function () {
		atom.project.divinefingers.on('disconnected', function () {
			self.showOffline();
		});
	};
	...
	TreeView.prototype.attach = function () {
		...
		/*if (atom.config.get('divinefingers.hideLocalWhenDisplayed'))
			hideLocalTree();
		else
			showLocalTree();*/
	};
	...
	TreeView.prototype.resolve = function (path) {
		var view = $('.divinefingers-view [data-path="'+ path +'"]').map(function () {
				var v = $(this).view();
				return v ? v : null;
			}).get(0);
	...
	TreeView.prototype.getSelected = function () {
		var views = $('.divinefingers-view .selected').map(function () {
				var v = $(this).view();
				return v ? v : null;
			}).get();
